...
if ($_GET['module']=='home'){
  ...
  $terkini=mysql_query("select count(komentar.id_komentar) as jml, judul, judul_seo, jam, 
                       ...
                       and aktif='Y' 
                       group by berita.id_berita DESC LIMIT 10");
...
// 	$terkini= mysql_query("SELECT * FROM berita ORDER BY id_berita DESC LIMIT 5");		 
    ...
    echo "$isi <a href=berita-$t[id_berita]-$t[judul_seo].html>Read More ...</a> (<b>$t[jml] komentar</b>)
          <br /></td></tr>
  ...
  if ($qsebelumnya > 0){
    ...
    echo "Sebelumnya<br />
          <ul>";
    $sebelum=mysql_query("SELECT * FROM berita ORDER BY id_berita DESC LIMIT 10,11");		 
...
elseif ($_GET['module']=='detailkategori'){
  ...
  // Tampilkan daftar berita sesuai dengan kategori yang dipilih
   ...
   while($r=mysql_fetch_array($hasil)){
    ...
    $isi = substr($isi_berita,0,220); // ambil sebanyak 220 karakter
    $isi = substr($isi_berita,0,strrpos($isi," ")); // potong per spasi kalimat
    echo "$isi <a href=berita-$r[id_berita]-$r[judul_seo].html>Read More ... </a>
...
elseif ($_GET['module']=='hasilcari'){
  ...
  if ($ketemu > 0){
    ...
    while($t=mysql_fetch_array($hasil)){
      ...
      echo "$isi <a href=berita-$t[id_berita]-$t[judul_seo].html>Read More ...</a>
            <br /></td></tr>
            </table><hr color=#e0cb91 noshade=noshade />";
    }                                                          
  }
...
elseif ($_GET['module']=='semuaberita'){
  ...
  while($r=mysql_fetch_array($sql)){
      ...
      echo "$isi <a href=berita-$r[id_berita]-$r[judul_seo].html>Read More ...</a> (<b>$r[jml] komentar</b>)
            </td></tr></table>
...
elseif ($_GET['module']=='semuadownload'){
  echo "<span class=judul_head>&#187; <b>Download Recipe</b></span><br />"; 
  $p      = new Paging5;
  $batas  = 25;
  $posisi = $p->cariPosisi($batas);
...
elseif ($_GET['module']=='hubungikami'){
  echo "<span class=judul_head>&#187; <b>Contact Us</b></span><br /><br />"; 
